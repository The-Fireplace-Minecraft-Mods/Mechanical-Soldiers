	...
	public void render(TileEntityPartConstructor te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		...
		if (te != null) {
			...
			if (fluid != null && fluid.getFluid() != null && fluid.getFluidAmount() > 0) {
				...
				FluidRenderer.renderFluid(fluid.getFluid(), te.getPos(), -0.05d, 0.0d, -0.05d, 0.1d, 0.0d, 0.1d, 1.0d, (double) fluid.getFluidAmount() / (double) fluid.getCapacity() * 0.375d, 1.0d, fluid.getFluid().getFluid().getColor());
